...
class TestExpenseCategoryAPI:
    ...
    def test_list_categories_authenticated(self, authenticated_client, expense_category):
        ...
        assert response.data['results'][0]['name'] == 'Alquiler Local'
...
class TestExpenseAPI:
    ...
    def test_list_expenses_unauthorized(self, api_client):
        """GET /api/v1/expenses/expenses/ sin autenticación retorna 401."""
        response = api_client.get('/api/v1/expenses/expenses/')
        assert response.status_code == status.HTTP_401_UNAUTHORIZED

    def test_list_expenses_authorized(self, authenticated_client, expense):
        """GET /api/v1/expenses/expenses/ con autenticación."""
        response = authenticated_client.get('/api/v1/expenses/expenses/')
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data['results']) > 0

    def test_list_expenses_filter_by_category(self, authenticated_client, expense, user):
        """GET /api/v1/expenses/expenses/?category_type=rent"""
        ...
        response = authenticated_client.get('/api/v1/expenses/expenses/?category_type=rent')
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_list_expenses_filter_by_paid_status(self, authenticated_client, expense, expense_paid):
        """GET /api/v1/expenses/expenses/?is_paid=True"""
        response = authenticated_client.get('/api/v1/expenses/expenses/?is_paid=True')
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data['results']) == 1
    ...
    def test_create_expense(self, authenticated_client, expense_category):
        ...
        response = authenticated_client.post('/api/v1/expenses/expenses/', payload)
        assert response.status_code == status.HTTP_201_CREATED
        assert response.data['description'] == 'Nuevo gasto'
        assert response.data['amount_total'] == '5000.00'

    def test_create_expense_with_iva(self, authenticated_client, expense_category):
        """POST /api/v1/expenses/expenses/ con IVA."""
        payload = {
            'category_id': expense_category.id,
        ...
        response = authenticated_client.post('/api/v1/expenses/expenses/', payload)
        assert response.status_code == status.HTTP_201_CREATED
        assert response.data['amount_iva'] == '210.00'
        assert response.data['amount_total'] == '1210.00'
    ...
    def test_retrieve_expense(self, authenticated_client, expense):
        ...
        response = authenticated_client.get(f'/api/v1/expenses/expenses/{expense.id}/')
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_update_expense(self, authenticated_client, expense):
        """PUT /api/v1/expenses/expenses/{id}/"""
        payload = {
        ...
        response = authenticated_client.put(f'/api/v1/expenses/expenses/{expense.id}/', payload)
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_partial_update_expense(self, authenticated_client, expense):
        ...
        response = authenticated_client.patch(f'/api/v1/expenses/expenses/{expense.id}/', payload)
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_delete_expense(self, authenticated_client, expense):
        ...
        exp_id = expense.id
        response = authenticated_client.delete(f'/api/v1/expenses/expenses/{exp_id}/')
    ...
    def test_mark_as_paid(self, authenticated_client, expense):
        """POST /api/v1/expenses/expenses/{id}/mark_as_paid/"""
        payload = {'payment_date': '2026-05-10'}
        response = authenticated_client.post(
            f'/api/v1/expenses/expenses/{expense.id}/mark_as_paid/',
            payload
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_unpaid_expenses_endpoint(self, authenticated_client, expense, expense_paid):
        """GET /api/v1/expenses/expenses/unpaid/"""
        response = authenticated_client.get('/api/v1/expenses/expenses/unpaid/')
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data) == 1
        assert response.data[0]['is_paid'] is False

    def test_summary_endpoint(self, authenticated_client, expense_category, user):
        """GET /api/v1/expenses/expenses/summary/?from_date=...&to_date=..."""
        ...
        response = authenticated_client.get(
            '/api/v1/expenses/expenses/summary/?from_date=2026-05-01&to_date=2026-05-31'
        )
        assert response.status_code == status.HTTP_200_OK
    ...
    def test_summary_missing_params(self, authenticated_client):
        ...
        response = authenticated_client.get('/api/v1/expenses/expenses/summary/')
        assert response.status_code == status.HTTP_400_BAD_REQUEST

    def test_create_expense_permission_denied_operator_without_flag(self, api_client, expense_category):
        """Usuario sin can_manage_expenses y no admin/manager recibe 403."""
        ...
        response = api_client.post('/api/v1/expenses/expenses/', payload)
        assert response.status_code == status.HTTP_403_FORBIDDEN

    def test_mark_as_paid_invalid_date_format(self, authenticated_client, expense):
        """POST mark_as_paid con formato de fecha inválido retorna 400."""
        payload = {'payment_date': 'invalid-date'}
        response = authenticated_client.post(
            f'/api/v1/expenses/expenses/{expense.id}/mark_as_paid/',
            payload
        )
