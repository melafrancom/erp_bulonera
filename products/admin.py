...
class ProductAdmin(admin.ModelAdmin):
    ...
    list_editable = ['price', 'cost', 'is_active']
    prepopulated_fields = {'slug': ('name',)}
    ...
    def _handle_excel_import(self, request, template_name, title, is_price_update=False):
        """Helper para procesar archivos de importación/actualización en admin."""
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                file_name = f"admin_import_{timestamp}_{safe_name}"
                ...
                try:
                    ...
                    if result['errors']:
                        error_sample = [
                            f"Fila {e['row']} ({escape(str(e['code']))}): {escape(str(e['error']))}"
                        ...
                        msg_prefix = "Precios actualizados" if is_price_update else f"Importados {result['successful']} productos, pero {result['failed']} fallaron."
                        messages.warning(
                            request,
                            f"{msg_prefix}<br>{error_msg}",
                            extra_tags='safe'
                        )
                    else:
                        if is_price_update:
                            messages.success(
                                request,
                                f"Precios actualizados correctamente para {result['successful']} productos."
                            )
                        else:
                            messages.success(
                                request,
                    ...
                    messages.error(request, f"Error al procesar: {escape(str(e))}")
                finally:
    ...
    def import_products_view(self, request):
        ...
        return self._handle_excel_import(
            request,
            'admin/products/import_products.html',
            'Importar Productos',
    ...
    def update_prices_view(self, request):
        ...
        return self._handle_excel_import(
            request,
            'admin/products/update_prices.html',
            'Actualizar Precios',
