...
def manager_required(view_func):
    """
    Decorador que verifica que el usuario sea manager, admin o superuser.
...
def permission_required(perm_name):
    ...
    o es administrador/manager/superuser. Lanza 403 si falla.
    """
    def decorator(view_func):
        ...
        def _wrapped_view(request, *args, **kwargs):
            ...
            if request.user.is_superuser or request.user.role in ('admin', 'manager') or getattr(request.user, perm_name, False):
                return view_func(request, *args, **kwargs)
