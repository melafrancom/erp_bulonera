...
*   `CuentaCorrienteService`: Orquestador central de la cuenta corriente (optimizado con consultas SQL agregadas `annotate(Sum(Case(When(...))))` para evitar N+1 queries):
