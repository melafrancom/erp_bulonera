...
*   `cancel_sale(sale, user, reason)`: Anula una venta no entregada. Libera automáticamente todas las alocaciones de pago activas (`PaymentAllocation` soft-deleted), recalculando el estado de cobro, y devuelve los artículos al stock disponible si ya estaban en estado `ready`.
...
*   **Seguridad en Sincronización PWA (`sync/upload/` y `sync/resolve/`)**: Se aplica restricción estricta de pertenencia (`created_by=request.user`) para evitar vulnerabilidades IDOR. `_sync_single_sale` asigna automáticamente `unit_cost=product.current_cost` si no viene provisto. Los datos fusionados mediante `client_wins` excluyen el campo `status` para evitar transiciones no permitidas en la máquina de estados.
