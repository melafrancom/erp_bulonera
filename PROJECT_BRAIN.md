...
- **Suite de Tests Global:** 480+ tests ejecutados y aprobados (**100% PASSED**) en Docker.
...
- **Módulo `bills`:** Remediado al 100%. Facturación directa de 0, edición de nombres con código inmutable, notas de crédito standalone, aplicación de `ModulePermissionRequiredMixin` y `@permission_required('can_manage_bills')` en vistas web, inmutabilidad fiscal en Admin Django (`has_delete_permission = False` para `autorizadas`), preservación de descarga pública por UUID (`invoice_public_pdf`) y 40 tests pasando.
...
- **Módulo `core`:** Sincronización automática de permisos en `User.save()`, comprobación de unicidad en `User.restore()`, superuser bypass en decoradores web (`@manager_required`) y creación del Mixin CBV `ModulePermissionRequiredMixin` en `core/decorators.py`.
- **PWA e Integridad Offline (Fase 8):** Remediada al 100%. Validación cruzada de precios con el catálogo activo en `sync_views.py` (`PRICE_TOLERANCE = 0.05`), marcando ventas con desvío como `conflict` en `internal_notes` y `warnings` sin trabar la venta en mostrador; aislamiento de rutas fiscales y financieras en `static/service-worker.js` (`v2.0.2`, `SENSITIVE_API_PATTERNS` a `Network Only`); estandarización de `static/js/offline-db.js` con endpoint canónico `/api/v1/sales/sync/upload/`, soporte de `EnvelopeRenderer` (`_extractResults`) y UUID v4; purga automática de IndexedDB (`bulonera-erp-offline`) al confirmar logout en `logout_confirm.html`; y 75/75 tests de `sales` pasando.
