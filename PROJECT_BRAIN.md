...
| [bills/README.md](bills/README.md) | Documentación y arquitectura del módulo de Facturación Legal y ARCA |
| [payments/README.md](payments/README.md) | Documentación y arquitectura del módulo de Pagos e Imputaciones |

---

## 🧪 Estado de Pruebas e Infraestructura (Agosto / Septiembre 2026)
- **Edición Retroactiva de Costos en Ventas Confirmadas (`sales`):**
  1. *Lógica de Negocio y Auditoría:* Servicio `update_sale_item_costs` en `sales/services.py` con `@transaction.atomic` y `select_for_update()` sobre `Sale` y `SaleItem`. Permite corregir o registrar costos unitarios (`unit_cost`) retroactivamente sin anular la venta, recalculando en vivo el margen/ganancia (`profit`) de cada renglón.
  2. *Trazabilidad:* Incorpora automáticamente una entrada con marca de tiempo, usuario y justificación en `sale.internal_notes`.
  3. *Invalidación Automática de Estados:* Invalida de forma reactiva el `FinancialSnapshot` del período contable de la venta (`is_stale=True`).
  4. *Vista Web y Modal Alpine.js:* Vista `sale_update_costs` (`/ventas/ventas/<pk>/editar-costos/`) y modal interactivo en `sale_detail.html` exclusivo para administradores y gerentes, con cálculo en tiempo real de costo total y ganancia estimada.
  5. *Cobertura:* Suite completa de tests en `sales/tests/test_sale_cost_editing.py`.
- **Corrección Integral del Motor de Reportes y Flujos de Caja (`reports`):**
  1. *Sincronización de Señales:* `reports/signals.py` ahora escucha mutaciones de `Sale` y `SaleItem` (`post_save` / `post_delete`), invalidando los snapshots de P&L del mes correspondiente ante cualquier corrección de venta o costo.
  2. *Control de Frescura y Recálculo en Vivo:* En `financial_views.py`, los datos del mes en curso o con `?refresh=1` se calculan siempre frescos vía `ProfitAndLossService` y `CashFlowService`, evitando cacheos obsoletos.
  3. *Soporte Flexible de Períodos:* Función `_parse_period_from_request` que procesa tanto `?period=YYYY-MM` como `?year=Y&month=M`.
  4. *Evolución Mensual Anual:* Inclusión asegurada del mes corriente en los gráficos de evolución de 12 meses (`_add_monthly_evolution` y `_add_monthly_evolution_cashflow`).
  5. *Toolbar en Plantillas:* Selector con `onchange="this.form.submit()"`, botón directo de recálculo en tiempo real ("🔄 Recalcular") y exportador a Excel.
- **Experiencia de Usuario en Mostrador & Layout Fluido (Completada):**
  ...
  5. *Layout 100% Full-Width en Facturación Directa:* Formulario apilado verticalmente en `invoice_form.html` para máxima legibilidad.
